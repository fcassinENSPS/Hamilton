...
def recupHamilton():
	...
	for i in tqdm(range(int(entrnb.get()))):
		...
		#plt.plot(b)
	"""
	h = Hamilton(float(entrB.get()),float(entrA.get()),float(entrrho.get()),int(entrnb.get()),int(entrM.get()),float(entrdx.get()),float(entrdy.get()),float(entrT.get()),float(entrdt.get()),choixInt.get(),int(entrnth.get()))
	x,y = h.dynamicsb()
	#plt.plot(x,y,'.')
	plt.plot(np.mod(x,2*np.pi),np.mod(y,2*np.pi),'.')
...
bouton.grid(row = 10, columnspan = 2)
